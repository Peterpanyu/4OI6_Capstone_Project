...
int32_t hx711_value_ave(hx711_t *hx711, uint16_t sample)
{
  ...
  int32_t answer = (int32_t)(ave);
  hx711_unlock(hx711);
  return answer;
}
//#############################################################################################
void hx711_tare(hx711_t *hx711, uint32_t sample)
{
  hx711_lock(hx711);
  uint32_t  ave = hx711_value(hx711);
	hx711_delay(10);
  for(uint16_t i=0 ; i<sample ; i++)
  {
		ave += hx711_value(hx711);
